...
Absorb the standalone `snowplow-tracking-cli` (currently at `github.com/snowplow/snowplow-tracking-cli`, v0.7.0, Apache 2.0) into this repository as a new `events` (alias `track`) command family. The result: one Snowplow CLI binary that can both manage tracking design via Console (existing `data-products` / `data-structures`) *and* send events to a collector (new `events send`). The standalone repo is then archived.
...
1. All current `snowplow-tracking-cli` functionality is available as `snowplow-cli events send ...` with no loss of capability.
...
```
snowplow-cli events                    # alias: track
snowplow-cli events send [flags]       # primary command — sends one event
```

`send` is the only leaf at launch. We pick `events` as the family name so we can grow it later (e.g. `events bulk-send`, `events replay`) without renaming. The `track` alias matches the old binary name for muscle memory.

### Flags on `events send`
...
Implementation: leaf `RunE` returns a typed error and `cmd.Execute()` maps it to the right exit code. May require a small wrapper in `cmd.Execute()` since today it just does `os.Exit(1)` on any error — see "Implementation plan" step 5.
...
cmd/
  events/                         NEW package
    ...
    send.go                         The `send` leaf
    send_test.go                    Port of snowplowtrk_test.go
internal/
  tracker/                        NEW package
...
- A `CHANGELOG` entry for the release that introduces `events send`, explicitly crediting the upstream tool.
...
7. **Deprecate the upstream repo.** Once the first release of `snowplow-cli` containing `events send` is out: rewrite `snowplow-tracking-cli`'s README to point at this binary, push a final v0.8.0 release with the deprecation notice, and archive the GitHub repo. Push a final `snowplow/snowplow-tracking-cli:deprecated` Docker tag with the same README.

## Distribution

No new artefacts needed. The existing release pipeline (`cd.yaml` → GitHub Releases, Homebrew, npm) ships the merged binary automatically. Users invoke `snowplow-cli events send ...` instead of running a separate binary.
...
# After
snowplow-cli events send --collector c.acme.com --schema iglu:.../1-0-0 --json '{"hello":"world"}'
```
...
3. **GET-default flip** — comfortable doing this as a silent behaviour change in the migrated tool, or do we want `events send` to *error* if `--method` is omitted, for one release, to force users to make an explicit choice?
4. **Family name** — `events` vs `track` vs `tracker`. Spec assumes `events` with `track` as alias; happy to swap.
5. **MCP exposure** — should `events send` be exposed as an MCP tool? Probably yes (sending a test event from an LLM agent is a natural use case) but out of scope for the initial merge.
